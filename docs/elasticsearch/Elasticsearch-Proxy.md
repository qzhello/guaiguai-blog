...
2. 动态参数配置、动态线程池（写入）。 
3. 批量写入。rocketmq、kafka异步数据消费。 内存消费熔断or清空？
   压缩。索引、集群纬度数据压缩。 
...
4. 优化配置项。
    ...
    - 快速失败，防止故障时难以恢复
5. 就近写入，防止data节点之间发送。 
6. 双中心。消息如何保证不限流。保证主从数据的最终一致性。数据读写分离，索引/模版级别。结构check。p0级别索引，主从自动数据切换？
故障期间，突增流量的AI分析，快速告警，措施
7. 可观测性。监控与告警。提供标准/metrics，接入prometheus，带上ident，环境，自定义标签。hot_threads。
...
14. 备份。s3快照，恢复。
15. k8s部署。独享、共享。
16. 不同集群，不同索引的cache是否可以调整，细化资源。
17. 治理。reindex别名？看怎么好实现。还有冷冻，把多个分片搞成一个。
18. 自定义分词。
19. 堆外接口。提供分词接口，分析接口。自定义分词接口。
